...
- **The test fixture copies asset dirs in two places.** A new top-level asset directory must be added to *both* the copy loop in `tests/run.sh` (the run-wide fixture) and the one in `tests/lib.sh`'s `make_fixture` (the per-test mutable fixture). Miss `run.sh` and the new type's assets silently never reach the fixture, so its setup tests fail as if the files were missing.
- **Interview overlays are keyed by overlay name, not project type.** `setup.sh` resolves the overlay via `overlay_for "$PROJECT_TYPE"`, so several types can share one file — game types (`godot`, `love2d`) all map to `interviews/overlays/game.md`. A type with no `overlay_for` case gets no overlay.
