...
test_common_asset_dir_mapping() {
  ...
  assert_eq "$(asset_dir_for puppet)"   "puppet"  "puppet maps to puppet"
...
test_common_overlay_mapping() {
  ...
  assert_eq "$(overlay_for puppet)"   "puppet" "puppet has its own overlay"
...
test_common_project_types_contract() {
  ...
  assert_match " ${PROJECT_TYPES[*]} " " puppet "
}
